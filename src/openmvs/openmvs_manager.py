...
class OpenmvsManager:
    ...
    def reconstruct_mesh(self, input_mvs: Path, output_ply: Path):
        ...
        command = [
            ...
            # Rimosso "-d", "10.0" perché duplicato di --min-point-distance
            ...
            "--max-threads", "12"          # Riduciamo i thread a 12
        ]

        try:
            print(f"⚙️  Avvio ReconstructMesh su CPU (20 core)...")
            subprocess_execution(command, 'Reconstructing mesh.', output_log=self.output_log, cwd=working_dir)
